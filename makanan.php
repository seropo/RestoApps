...
$('#cx').fadeOut(1000);					$('#paket-makanan').find('li').eq(4).on('click',function(){
 $('#black-transpare').fadeIn(1000);window.alert('value: $('#paket-makanan').find('li').index(this)');
 $('#btn-cloes').fadeIn(1000);
 $('#pesanan').fadeIn(1000);
 $('#pesanan input[name=namamakanan]').val('kentang');
 $('#pesanan input[type=number]').on('keyup mouseup',function(){

 $('#value').text($('#pesanan input[type=number]').val() *10000);
  });
...
isset($_POST["jekel"])||isset($_POST["alamat"])||isset($_POST["kodepost"])){
 ...
 }else if(isset($_POST["namamakanan"]) && isset($_POST["jumlahmakanan"])){
	 $namamakanan=$_POST["namamakanan"];
	 $jumlah=$_POST["jumlahmakanan"];
	isset($_SESSION["check"])?
	$val=$_SESSION["check"] : $val="1";
		 $harga=$jumlah*10000;
	 echo "<script> makanan() </script>";
	$cx=$controller->pesanmakanan($val,$namamakanan,$jumlah,$harga);
